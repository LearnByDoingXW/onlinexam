		...
		<div>
			<form id="form1" name="form1" method="post" action="<%=path %>/studentTest?&testId=${test.id}">
				...
				<c:forEach var="que" items="${quesList}" varStatus="status">
					<p></p>
					<p>${status.index + 1}.${que.getQueTitle()}</p>
					...
					<p>
					A.<input type="radio" name = "ques_${que.getId()}" value = "A"/>
					B.<input type="radio" name = "ques_${que.getId()}" value = "B"/>
					C.<input type="radio" name = "ques_${que.getId()}" value = "C"/>
					D.<input type="radio" name = "ques_${que.getId()}" value = "D"/>
					</p>
				</c:forEach>
